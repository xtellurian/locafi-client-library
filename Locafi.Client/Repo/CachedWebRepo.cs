...
namespace Locafi.Client.Repo
{
    public abstract class CachedWebRepo : WebRepo
    {
        ...
        public async Task<ICachedResponse<bool>> Post<TData>(TData data, string extra = "", ICache<TData> cache = null) where TData : ICacheable, new()
        {
            try
            {
                // we need to check the types of exceptions thrown when offline and handle gracefully
                var result = await base.Post(data, extra);
                return new WebRepoCacheResult<bool>(result, true, false);
            }
            catch (Exception ex)
            {
                if (cache == null)
                {
                    return new WebRepoCacheResult<bool>(false, false, false, ex);
                }
                else
                {
                    var cacheEntity = new CachedEntity<TData>(data.Id, data, extra);
                    cache.Push(cacheEntity);
                    return new WebRepoCacheResult<bool>(false, false, true);
                }
            }
        }

        public async Task<IList<ICachedResponse<bool>>> PostCache<TData>(ICache<TData> cache, int? amount = null) where TData : ICacheable, new()
        {
            var list = new List<ICachedResponse<bool>>();
            foreach (var cachedEntity in cache.CopyCache(amount))
            {
                var result = await base.Post(cachedEntity.Entity, cachedEntity.Extra);
                if (result == false) break;
                list.Add(new WebRepoCacheResult<bool>(true, true, false));
